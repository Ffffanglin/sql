...
SELECT
  ...
  v_b_a.*
FROM vendor AS v
INNER JOIN vendor_booth_assignments AS v_b_a
  ON v.vendor_id = v_b_a.vendor_id
ORDER BY v.vendor_name, v_b_a.market_date;
...
SELECT
  ...
  SUM(c_p.quantity * c_p.cost_to_customer_per_qty) AS total_spent
FROM customer AS c
JOIN customer_purchases AS c_p
  ON c.customer_id = c_p.customer_id
GROUP BY c.customer_id, c.customer_last_name, c.customer_first_name
HAVING SUM(c_p.quantity * c_p.cost_to_customer_per_qty) > 2000
